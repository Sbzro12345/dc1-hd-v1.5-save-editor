...
- The plist is located under `/User/Library/Preferences/` or in `/User/Applications/[some random string]/Library/Preferences/`. You will probably need a jailbroken device to access and transfer the plist file. Alternatively, use TouchHLE. The files will be located under the sandbox dir, if using TouchHLE. The plist is located under `com.chillingo.defenderchronicleshd/Library/Preferences/` in TouchHLE. 
...
- BONUS: If on Android, you can pair TouchHLE with VPhoneOS so you can use GameGuardian on Defender Chronicles. This is useful for the Cheat Engine, as well as the speedhacks. You can also use Termux to run the Save Editor on Android. On newer Android versions, you will need to copy the save into a folder (with the save editor) outside `Android/data` due to storage protections using a Shizuku file explorer.
